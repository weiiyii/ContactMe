		...
		xhr.onreadystatechange = function()
		{
			if (this.readyState == 4 && this.status == 200)
			{
				...
				for( var i=0; i<jsonObject.results.length; i++ ){
					...
					// delete
					contactList += `<td id="td_5"><button type="button" id="deleteButton" class="btn btn-outline-primary" style="width: 80px border: 2px solid; font-weight:500" onclick="doDelete(${jsonObject.results[i]});"> Delete </button></td>`;
					// update
					contactList += `<td id="id_6"><button type="button" id="updateButton" class="btn btn-outline-danger" style="width:80px border: 2px solid; font-weight:500" onclick="updateContact(${jsonObject.results[i]},${(((i+1)/6)-1)});"> Update </button></td>\n</tr>\n`;
